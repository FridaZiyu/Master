...
Matrix Matrix::chol() {
  bool spd =
      (_R == _C); // spd is true when the matrix is symmetric positive definite
  if (!spd)
    throw std::logic_error("Matirx is not symmetric positive definite.");
  Matrix L(_R, _R);
  _flop = 0;
  for (int k = 0; k < _R; k++) {
    double s = 0;
    for (int i = 0; i < k; i++) {
      s += L.get(k, i) * L.get(k, i);
      _flop += 2;
    }
    s = _data[_R * k + k] - s;
    _flop++;
    ...
    if (!spd)
      throw std::logic_error("Matirx is not symmetric positive definite.");
    L.set(k, k, std::sqrt(s > 0 ? s : 0));
    _flop++;
    for (int i = k + 1; i < _R; i++) {
      s = 0;
      for (int j = 0; j < k; j++) {
        s += L.get(i, j) * L.get(k, j);
        _flop += 2;
      }
      L.set(i, k, (_data[_R * k + i] - s) / L.get(k, k));
      _flop += 2;
    }
  }
  return L;
}
int Matrix::getFlop() const { return _flop; }
